  ...
  async function saveClient() {
    ...
    try {
      const payload = {
        ...
        ...(editing ? { cliAtivo: selected?.active ? "A" : "I" } : { cliAtivo: "A", cliCpf: form.cpf.replace(/\D/g, "") }),
      }
    ...
    <DashboardShell title="Gerenciar Clientes" description="Cadastre, consulte e mantenha os registros das suas clientes atualizados.">
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 md:flex-row md:items-end">
          ...
          <Button onClick={startCreate} disabled={isSaving}><UserPlus className="mr-2 h-4 w-4" />Nova cliente</Button>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
          <Card>
            ...
            <CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="pb-3 font-medium">Cliente</th><th className="pb-3 font-medium">CPF</th><th className="pb-3 font-medium">Gênero</th><th className="pb-3 font-medium">Status</th><th className="pb-3 text-right font-medium">Ações</th></tr></thead><tbody>{filteredClients.map((client) => <tr key={client.id} className={`border-b last:border-0 ${selectedId === client.id ? "bg-secondary/40" : ""}`}><td className="py-4"><button type="button" onClick={() => setSelectedId(client.id)} className="text-left"><p className="font-medium">{client.name}</p><p className="font-mono text-[11px] text-muted-foreground">{client.id}</p></button></td><td className="py-4 text-muted-foreground">{client.cpf}</td><td className="py-4 text-muted-foreground">{client.gender}</td><td className="py-4"><Badge variant={client.active ? "default" : "secondary"}>{client.active ? "Ativa" : "Inativa"}</Badge></td><td className="py-4 text-right"><div className="flex justify-end gap-1"><Button size="icon" variant="ghost" disabled={isSaving} onClick={() => startEdit(client)} aria-label={`Editar ${client.name}`}><Pencil className="h-4 w-4" /></Button><Button size="icon" variant="ghost" disabled={isSaving} onClick={() => toggleStatus(client)} aria-label={`${client.active ? "Inativar" : "Ativar"} ${client.name}`}><Power className="h-4 w-4" /></Button></div></td></tr>)}</tbody></table>{isLoading ? <div className="py-12 text-center text-sm text-muted-foreground">Carregando clientes...</div> : filteredClients.length === 0 && <div className="py-12 text-center text-sm text-muted-foreground">Nenhum cliente encontrada com esses filtros.</div>}</div></CardContent>
          </Card>

          <Card className="h-fit">
            <CardHeader>
              <CardTitle className="text-base">{editing ? "Editar cliente" : selectedId !== null ? "Dados da cliente" : "Cadastrar cliente"}
            </CardTitle><p className="text-sm text-muted-foreground">{editing || selectedId === null ? "Atualize os dados abaixo." : "Selecione editar para alterar o registro."}</p>
            </CardHeader><CardContent className="flex flex-col gap-4">{selectedId !== null && !editing ? <><div><Label>ID do registro</Label>
            <Input value={selected?.id ?? ""} readOnly className="mt-2 bg-muted/40 font-mono text-xs" /></div><div><Label>Nome da cliente</Label>
            <Input value={selected?.name ?? ""} readOnly className="mt-2" /></div><div><Label>CPF</Label><Input value={selected?.cpf ?? ""} readOnly className="mt-2" /></div><div>
            <Label>Data de nascimento</Label>
            <Input value={selected?.birthDate ? new Date(`${selected.birthDate}T12:00:00`).toLocaleDateString("pt-BR") : ""} readOnly className="mt-2" /></div><div><Label>Gênero</Label>
            <Input value={selected?.gender ?? ""} readOnly className="mt-2" /></div><div className="flex items-center justify-between rounded-lg bg-muted/50 p-3"><div><p className="text-sm font-medium">Registro {selected?.active ? "ativo" : "inativo"}</p><p className="text-xs text-muted-foreground">Controle a disponibilidade do cadastro.</p></div>
            <Button variant="outline" size="sm" disabled={isSaving} onClick={() => selected && toggleStatus(selected)}><Power className="mr-2 h-3.5 w-3.5" />{selected?.active ? "Inativar" : "Ativar"}</Button></div><Button variant="outline" disabled={isSaving} onClick={() => selected && startEdit(selected)}><Pencil className="mr-2 h-4 w-4" />Editar dados</Button></> : <><div><Label htmlFor="client-form-name">Nome do cliente</Label><Input id="client-form-name" className="mt-2" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Nome completo" /></div><div><Label htmlFor="client-form-cpf">CPF</Label><Input id="client-form-cpf" className="mt-2" value={form.cpf} onChange={(event) => setForm((current) => ({ ...current, cpf: formatCpf(event.target.value) }))} placeholder="000.000.000-00" inputMode="numeric" /></div><div><Label htmlFor="client-form-birth">Data de nascimento</Label><Input id="client-form-birth" type="date" className="mt-2" value={form.birthDate} onChange={(event) => setForm((current) => ({ ...current, birthDate: event.target.value }))} /></div><div><Label htmlFor="client-form-gender">Gênero</Label><select id="client-form-gender" className="mt-2 flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm" value={form.gender} onChange={(event) => setForm((current) => ({ ...current, gender: event.target.value }))}>{genders.map((gender) => <option key={gender}>{gender}</option>)}</select></div><div className="flex gap-2"><Button variant="outline" className="flex-1" disabled={isSaving} onClick={() => { setEditing(false); setSelectedId(selected?.id ?? null) }}>Cancelar</Button><Button className="flex-1" disabled={isSaving} onClick={saveClient}>{isSaving ? "Salvando..." : editing ? "Salvar alterações" : "Cadastrar cliente"}</Button></div></>}
            </CardContent></Card>
        </div>
      </div>
    </DashboardShell>
